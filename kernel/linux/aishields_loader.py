#!/usr/bin/env python3
"""
AIShields Protect - eBPF Program Loader
...
logger = logging.getLogger("aishields.ebpf.loader")
...
class AIShieldsEBPFLoader:
    ...
    def run(self):
        ...
        logger.info("AIShields eBPF monitor running. Press Ctrl+C to stop.")
        while self.running:
            try:
...
def forward_to_control_plane(event: BPFEvent, url: str, api_key: str, tenant_id: str):
    """Forward eBPF events to the AIShields control plane."""
    import requests
...
def main():
    ...
    bpf_src = str(Path(__file__).parent / "aishields_monitor.bpf.c")
    config_path = os.environ.get("AISHIELDS_CONFIG", None)

    loader = AIShieldsEBPFLoader(bpf_src, config_path)
