"""CyberArmor.ai RASP — Python Runtime Application Self-Protection.
...
logger = logging.getLogger("aishields.rasp")

# ── Configuration ─────────────────────────────────────────
class RASPConfig:
    control_plane_url: str = os.getenv("CYBERARMOR_URL", "http://localhost:8000")
    api_key: str = os.getenv("CYBERARMOR_API_KEY", "")
    tenant_id: str = os.getenv("CYBERARMOR_TENANT", "default")
    mode: str = os.getenv("CYBERARMOR_MODE", "monitor")  # monitor | block
    ...
    def __call__(self, environ, start_response):
        ...
        if method == "POST" and is_ai_endpoint(url):
            ...
            if not result.allowed:
                ...
                return [json.dumps({"error": result.reason, "policy": "aishields-rasp"}).encode()]

        return self.app(environ, start_response)

# ── ASGI Middleware ────────────────────────────────────────
class CyberArmor.aiASGI:
...
def patch():
    ...
    try:
        ...
        def patched_request(self, method, url, **kwargs):
            if method.upper() == "POST":
                ...
                if not result.allowed:
                    raise PermissionError(f"CyberArmor.ai RASP blocked: {result.reason}")
            return _orig_request(self, method, url, **kwargs)
        requests.Session.request = patched_request
        logger.info("CyberArmor.ai RASP: patched requests.Session")
    except ImportError:
        pass
    ...
    try:
        ...
        logger.info("CyberArmor.ai RASP: patched httpx.Client")
    except ImportError:
        pass
...
# ── Auto-init ─────────────────────────────────────────────
def init(**kwargs):
    """Initialize CyberArmor.ai RASP with custom configuration."""
    for k, v in kwargs.items():
        if hasattr(config, k):
            setattr(config, k, v)
    patch()
    logger.info("CyberArmor.ai RASP initialized (mode=%s)", config.mode)
