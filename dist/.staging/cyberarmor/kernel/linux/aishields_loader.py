#!/usr/bin/env python3
"""
CyberArmor.ai Protect - eBPF Program Loader
...
class CyberArmor.aiEBPFLoader:
    ...
    def run(self):
        ...
        logger.info("CyberArmor.ai eBPF monitor running. Press Ctrl+C to stop.")
        while self.running:
            try:
...
def forward_to_control_plane(event: BPFEvent, url: str, api_key: str, tenant_id: str):
    """Forward eBPF events to the CyberArmor.ai control plane."""
    import requests
...
def main():
    ...
    loader = CyberArmor.aiEBPFLoader(bpf_src, config_path)
