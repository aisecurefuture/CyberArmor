"""
CyberArmor.ai Protect - IBM QRadar Output
...
class QRadarOutput(SIEMOutput):
    ...
    def _build_leef(self, event: dict[str, Any]) -> str:
        ...
        delimiter = "\t"
        header = (
            f"LEEF:2.0|CyberArmor.ai|Protect|"
    ...
    async def _post_reference_data(
        ...
        This enriches QRadar correlation rules with IOCs from CyberArmor.ai.
        """
        if not self._api_token:
            return

        ref_set_name = self._config.get(
            "QRADAR_REFERENCE_SET", "CyberArmor.ai_Events"
        )
        url = (
            ...
            async with httpx.AsyncClient(verify=False, timeout=15.0) as client:
                ...
                if resp.status_code == 200:
                    ...
                    if offenses:
                        ...
                        note_text = (
                            f"[CyberArmor.ai Protect] {event.get('severity', '').upper()}: "
        ...
        if self._config.get("QRADAR_SYSLOG_PORT"):
            try:
                test_msg = (
                    ...
                    f"aishields-protect siem-connector - test - "
                    f"LEEF:2.0|CyberArmor.ai|Protect|1.0.0|connectivity-test|"
                )
