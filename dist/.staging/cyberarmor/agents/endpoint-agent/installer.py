"""Cross-Platform Installer for CyberArmor.ai Endpoint Agent.
...
PRODUCT_NAME = "CyberArmor.ai Endpoint Agent"
...
INSTALL_PATHS = {
    ...
    "Windows": Path(os.environ.get("ProgramFiles", "C:\\Program Files")) / "CyberArmor.ai",
}

CONFIG_PATHS = {
    "Darwin": Path("/etc/cyberarmor"),
    "Linux": Path("/etc/cyberarmor"),
    "Windows": Path(os.environ.get("ProgramData", "C:\\ProgramData")) / "CyberArmor.ai",
}

LOG_PATHS = {
    "Darwin": Path("/var/log/aishields"),
    "Linux": Path("/var/log/aishields"),
    "Windows": Path(os.environ.get("ProgramData", "C:\\ProgramData")) / "CyberArmor.ai" / "logs",
}
...
SYSTEMD_UNIT = """[Unit]
Description=CyberArmor.ai Endpoint Agent
...
if ($nssm) {{
    ...
    & nssm set $serviceName Description "CyberArmor.ai AI Security Endpoint Agent"
    ...
    & sc.exe description $serviceName "CyberArmor.ai AI Security Endpoint Agent"
