"""AIShields RASP — Python Runtime Application Self-Protection.
...
# ── Configuration ─────────────────────────────────────────
class RASPConfig:
    control_plane_url: str = os.getenv("AISHIELDS_URL", "http://localhost:8000")
    api_key: str = os.getenv("AISHIELDS_API_KEY", "")
    tenant_id: str = os.getenv("AISHIELDS_TENANT", "default")
    mode: str = os.getenv("AISHIELDS_MODE", "monitor")  # monitor | block
...
def patch():
    ...
    try:
        ...
        def patched_request(self, method, url, **kwargs):
            if method.upper() == "POST":
                ...
                if not result.allowed:
                    raise PermissionError(f"AIShields RASP blocked: {result.reason}")
            return _orig_request(self, method, url, **kwargs)
        requests.Session.request = patched_request
        logger.info("AIShields RASP: patched requests.Session")
    except ImportError:
        pass
    ...
    try:
        ...
        logger.info("AIShields RASP: patched httpx.Client")
    except ImportError:
        pass
...
# ── Auto-init ─────────────────────────────────────────────
def init(**kwargs):
    """Initialize AIShields RASP with custom configuration."""
    for k, v in kwargs.items():
        if hasattr(config, k):
            setattr(config, k, v)
    patch()
    logger.info("AIShields RASP initialized (mode=%s)", config.mode)
