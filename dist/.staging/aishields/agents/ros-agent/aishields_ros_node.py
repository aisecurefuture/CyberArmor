"""AIShields ROS2 Security Node -- Main entry point.
...
to ``/aishields/events``, synchronises policies with the AIShields control
...
# Logging
# ---------------------------------------------------------------------------
logger = logging.getLogger("aishields.ros_node")
...
EVENTS_TOPIC = "/aishields/events"
STATUS_TOPIC = "/aishields/status"
...
class AIShieldsROSNode(Node):
    """Central security node for AIShields ROS2 integration.

    Responsibilities:
    * Publish security events on ``/aishields/events`` (JSON-encoded
      ``std_msgs/String``).
    ...
    * Coordinate sub-modules (topic_monitor, service_guard, etc.).
    ...
    def __init__(self) -> None:
        super().__init__("aishields_security_node")
        ...
        self.get_logger().info(
            "AIShields ROS2 Security Node v%s started  agent_id=%s",
            AGENT_VERSION,
            self._agent_id,
        )
    ...
    def publish_event(
        ...
        """Publish a security event on /aishields/events."""
    ...
    def destroy_node(self) -> None:
        self._running = False
        self.get_logger().info("AIShields ROS2 Security Node shutting down")
        super().destroy_node()
