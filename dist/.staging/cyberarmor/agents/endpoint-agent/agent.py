"""CyberArmor.ai Endpoint Agent - Cross-platform system service daemon.
...
CONTROL_PLANE_URL : str
    Base URL of the CyberArmor.ai control plane (default ``https://localhost:8000``).
AGENT_API_KEY : str
...
* Windows: ``%PROGRAMDATA%\\CyberArmor.ai\\agent.conf``
"""
...
def _config_path() -> Path:
    ...
    if system == "Windows":
        ...
        return Path(program_data) / "CyberArmor.ai" / "agent.conf"
    return Path("/etc/cyberarmor/agent.conf")
...
class EndpointAgent:
    """CyberArmor.ai cross-platform endpoint agent daemon.
    ...
    async def start(self) -> None:
        ...
        logger.info(
            "Starting CyberArmor.ai Endpoint Agent v%s  agent_id=%s",
            AGENT_VERSION,
            self.agent_id,
        )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="CyberArmor.ai Endpoint Agent")
    parser.add_argument(
        "--config",
        type=Path,
        default=None,
