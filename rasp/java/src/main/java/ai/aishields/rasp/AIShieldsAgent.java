/*
 * AIShields Protect - Java RASP Agent
 ...
 *
 * Usage: -javaagent:aishields-rasp.jar[=config.properties]
 *
 * Copyright (c) 2026 AIShields, Inc. All rights reserved.
 */
package ai.aishields.rasp;
...
/**
 * AIShieldsAgent is a Java Instrumentation agent that intercepts outbound HTTP
 * requests to AI/LLM services, enforces DLP policies, detects prompt injection,
 * and reports telemetry asynchronously to the AIShields control plane.
 */
public final class AIShieldsAgent {

    private static final Logger LOG = Logger.getLogger("ai.aishields.rasp");
    ...
    public static void premain(String agentArgs, Instrumentation inst) {
        if (!initialized.compareAndSet(false, true)) {
            LOG.warning("AIShields RASP agent already initialized; skipping duplicate premain.");
            return;
        }

        LOG.info("AIShields RASP agent initializing...");
        ...
        try {
            ...
            inst.addTransformer(new AIShieldsTransformer(), inst.isRetransformClassesSupported());

            // Register shutdown hook
            Runtime.getRuntime().addShutdownHook(new Thread(AIShieldsAgent::shutdown, "aishields-shutdown"));

            LOG.info("AIShields RASP agent initialized successfully. Mode: " + config.enforcementMode);
        } catch (Exception ex) {
            LOG.log(Level.SEVERE, "Failed to initialize AIShields RASP agent", ex);
        }
    }
    ...
    private static void startTelemetryFlush() {
        telemetryExecutor = Executors.newSingleThreadScheduledExecutor(r -> {
            Thread t = new Thread(r, "aishields-telemetry");
            t.setDaemon(true);
            return t;
        });
        telemetryExecutor.scheduleWithFixedDelay(
                AIShieldsAgent::flushTelemetry,
    ...
    private static void startPolicySync() {
        policySyncExecutor = Executors.newSingleThreadScheduledExecutor(r -> {
            Thread t = new Thread(r, "aishields-policy-sync");
            t.setDaemon(true);
            return t;
        });
        policySyncExecutor.scheduleWithFixedDelay(
                AIShieldsAgent::syncPolicies,
                0,
    ...
    static final class AIShieldsTransformer implements ClassFileTransformer {
        ...
        public byte[] transform(ClassLoader loader, String className,
            ...
            if (TARGET_CLASSES.contains(className)) {
                LOG.fine("AIShields intercepting class: " + className);
                ...
                // in the AIShieldsHttpInterceptor can find it.
            }
    ...
    private static void setupLogging() {
        Logger rootLogger = Logger.getLogger("ai.aishields");
    ...
    static final class AgentConfig {
        ...
        static AgentConfig fromProperties(Properties props) {
            String url = props.getProperty("aishields.controlPlane.url", "https://api.aishields.ai");
            String key = props.getProperty("aishields.apiKey", "");
            String id = props.getProperty("aishields.agentId", UUID.randomUUID().toString());
            EnforcementMode mode;
            try {
                mode = EnforcementMode.valueOf(
                        props.getProperty("aishields.mode", "MONITOR").toUpperCase());
            } catch (IllegalArgumentException e) {
                mode = EnforcementMode.MONITOR;
            }
            long telemetryInterval = Long.parseLong(
                    props.getProperty("aishields.telemetry.flushIntervalMs", "5000"));
            long policyInterval = Long.parseLong(
                    props.getProperty("aishields.policy.syncIntervalMs", "60000"));
            Level level;
            try {
                level = Level.parse(props.getProperty("aishields.logLevel", "INFO"));
            } catch (IllegalArgumentException e) {
                level = Level.INFO;
            }
            String logFile = props.getProperty("aishields.logFile", null);
            String customEndpoints = props.getProperty("aishields.customEndpoints", "");
        ...
        static AgentConfig applyEnvironmentOverrides(AgentConfig base) {
            String envUrl = System.getenv("AISHIELDS_CONTROL_PLANE_URL");
            String envKey = System.getenv("AISHIELDS_API_KEY");
            String envMode = System.getenv("AISHIELDS_MODE");
            String envId = System.getenv("AISHIELDS_AGENT_ID");
     ...
     *       .addInterceptor(AIShieldsAgent.okHttpInterceptor())
     *       .build();
     * </pre>
     *
    ...
    public static Object okHttpInterceptor() {
        return java.lang.reflect.Proxy.newProxyInstance(
                AIShieldsAgent.class.getClassLoader(),
    ...
    private static Object handleOkHttpIntercept(Object chain) throws Exception {
        ...
        if (!result.allowed) {
            ...
            throw new SecurityException("AIShields RASP blocked request: " + result.blockReason);
        }
    ...
    /**
     * Wraps a standard {@link HttpURLConnection} with AIShields inspection.
     * Usage:
     * <pre>
     *   HttpURLConnection conn = (HttpURLConnection) url.openConnection();
     *   conn = AIShieldsAgent.wrapConnection(conn);
     *   // proceed as normal
     * </pre>
     */
        ...
        public OutputStream getOutputStream() throws IOException {
            ...
            return new FilterOutputStream(original) {
                ...
                public void close() throws IOException {
                    ...
                    if (!result.allowed) {
                        throw new IOException("AIShields RASP blocked request: " + result.blockReason);
                    }
                    super.close();
                }
            };
