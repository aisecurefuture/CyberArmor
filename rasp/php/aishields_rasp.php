<?php
/**
 * AIShields RASP — PHP Runtime Application Self-Protection
...
class Config {
    ...
    public function __construct() {
        $this->controlPlaneUrl = getenv('AISHIELDS_URL') ?: 'http://localhost:8000';
        $this->apiKey = getenv('AISHIELDS_API_KEY') ?: '';
        $this->tenantId = getenv('AISHIELDS_TENANT') ?: 'default';
        $this->mode = getenv('AISHIELDS_MODE') ?: 'monitor';
...
class Inspector {
    ...
    public function inspect(string $url, string $body = ''): InspectionResult {
        ...
        if ($this->config->promptInjectionEnabled && $body) {
            foreach ($this->promptInjectionPatterns as $pattern) {
                if (preg_match($pattern, $body)) {
                    error_log("[AIShields RASP] Prompt injection detected: $pattern");
        ...
        if ($this->config->dlpEnabled && $body) {
            ...
            if (!empty($findings)) {
                error_log("[AIShields RASP] Sensitive data: " . implode(',', $findings));
                if ($this->config->mode === 'block') {
            ...
            if ($this->inspector->isAiEndpoint($host)) {
                ...
                if (!$result->allowed) {
                    $response = new \Nyholm\Psr7\Response(403, ['Content-Type' => 'application/json'],
                        json_encode(['error' => $result->reason, 'policy' => 'aishields-rasp']));
                    return $response;
                }
            }
        }
    ...
    public function handle($request, \Closure $next) {
        if ($request->isMethod('POST')) {
            ...
            if ($this->inspector->isAiEndpoint($host)) {
                ...
                if (!$result->allowed) {
                    return response()->json(['error' => $result->reason, 'policy' => 'aishields-rasp'], 403);
                }
            }
        }
        return $next($request);
