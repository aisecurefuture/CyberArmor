# Azure App Registration Setup for AIShields Protect

This guide walks through configuring Microsoft Entra ID (formerly Azure AD) as an identity provider for AIShields Protect.

## Prerequisites

- Azure subscription with Global Administrator or Application Administrator role
- AIShields Protect control plane deployed and accessible
- TLS certificate configured for your AIShields domain
...
3. Configure:
   - **Description**: `AIShields Protect Production`
...
## Step 5: Configure App Roles (for AIShields RBAC)
...
|-------------|-------|-------------|-----------------|
| AIShields Admin | `aishields.admin` | Full administrative access to AIShields Protect | Users/Groups |
| AIShields Analyst | `aishields.analyst` | View incidents, telemetry, and compliance reports | Users/Groups |
| AIShields Policy Manager | `aishields.policy_manager` | Create and manage security policies | Users/Groups |
| AIShields Viewer | `aishields.viewer` | Read-only access to dashboards | Users/Groups |
| AIShields API Client | `aishields.api_client` | Programmatic API access for service accounts | Applications |

## Step 6: Assign Users and Groups

1. Go to **Enterprise applications** > **AIShields Protect**
...
4. Recommended group structure:
   - `SG-AIShields-Admins` → `aishields.admin`
   - `SG-AIShields-Analysts` → `aishields.analyst`
   - `SG-AIShields-PolicyMgrs` → `aishields.policy_manager`
   - `SG-AIShields-Viewers` → `aishields.viewer`

## Step 7: Configure Conditional Access (Recommended)

1. Go to **Microsoft Entra ID** > **Security** > **Conditional Access**
2. Create a new policy:
   - **Name**: `AIShields Protect - Require MFA`
   - **Assignments**: Target the AIShields Protect application
   - **Conditions**: All client apps
   - **Grant**: Require multifactor authentication
   - **Session**: Sign-in frequency = 12 hours
3. Enable the policy

## Step 8: Configure AIShields Identity Service

Update your AIShields deployment configuration:
...
1. Navigate to your AIShields dashboard
2. Click **Sign in with Microsoft**
3. Authenticate with your Entra ID credentials
4. Verify the correct role is mapped in the AIShields admin panel under **Identity** > **Sessions**
...
# Call AIShields API with the token
...
) -DisplayName "AIShields Token Policy" -Type "TokenLifetimePolicy"
...
For SaaS deployments where multiple organizations use AIShields:

1. Change the app registration to **Multitenant** (Accounts in any organizational directory)
2. Implement tenant allow-listing in the AIShields Identity service
3. Configure per-tenant policy mappings
4. Enable tenant isolation in the AIShields control plane
