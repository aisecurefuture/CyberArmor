...
import { CyberArmor.aiWebSocket } from "../services/websocket";
import { TelemetryEvent } from "../types";

export function useWebSocket(serverUrl: string, apiKey: string, tenantId: string) {
  const [connected, setConnected] = useState(false);
  const [events, setEvents] = useState<TelemetryEvent[]>([]);
  const wsRef = useRef<CyberArmor.aiWebSocket | null>(null);

  useEffect(() => {
    if (!serverUrl || !apiKey || !tenantId) return;

    const ws = new CyberArmor.aiWebSocket(serverUrl, apiKey, tenantId);
    wsRef.current = ws;
