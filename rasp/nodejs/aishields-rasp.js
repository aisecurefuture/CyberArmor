/**
 * AIShields RASP — Node.js Runtime Application Self-Protection
...
// ── Config ───────────────────────────────────────────────
const config = {
  controlPlaneUrl: process.env.AISHIELDS_URL || 'http://localhost:8000',
  apiKey: process.env.AISHIELDS_API_KEY || '',
  tenantId: process.env.AISHIELDS_TENANT || 'default',
  mode: process.env.AISHIELDS_MODE || 'monitor', // monitor | block
  dlpEnabled: true,
...
function patch() {
  ...
  console.log('[AIShields RASP] HTTP modules patched');
}
...
function init(opts = {}) {
  ...
  patch();
  console.log(`[AIShields RASP] Initialized (mode=${config.mode})`);
}
